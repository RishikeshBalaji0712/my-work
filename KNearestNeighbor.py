...
def KNN(st):
    df = num('/Users/Rishikesh/Documents/Internship/Robotech Solutions/Data1.xlsx')
    features = df.values[:, 5 : 9].tolist()
    result = df.values[:, 9 : 13].tolist()
    model = KNeighborsClassifier(n_neighbors = 100)
    model.fit (features, result)
    _df_ = num(st)
    testdata = _df_.values[:, 5 : 9].tolist()
    predval = (model.predict(testdata))
    actval = _df_.values[:, 9 : 13]
    acttype = _df_.values[:, 4]
    predtype = NumToStr(predval)
    _df_['Predicted Types'] = predtype
    return _df_
